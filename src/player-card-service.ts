...
export class PlayerCardService {
  ...
  private async renderSvg(model: PlayerCardModel): Promise<string> {
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="1000" viewBox="0 0 1280 1000">
      <style>
      ...
      <rect class="card" x="20" y="260" width="1240" height="330" rx="8"/><text x="640" y="295" text-anchor="middle" class="section">最近顺位数据（旧 → 新）</text><g transform="translate(175 315)">${trend}</g><rect x="20" y="470" width="1240" height="94" rx="8" fill="#1d4ed8"/>${renderPromotion(model)}
      <rect class="card" x="20" y="610" width="590" height="370" rx="8"/><text x="315" y="645" text-anchor="middle" class="section">能力分析</text><g transform="translate(30 650)">${radar}</g>
      <rect class="card" x="630" y="610" width="630" height="370" rx="8"/><text x="945" y="645" class="section" text-anchor="middle">顺位分布</text><g transform="translate(660 650)">${placement}</g>${renderPlacementLegend(model)}
    </svg>`
  }
}
